...
import com.forum.async.MailTask;
import com.forum.mapper.UserMapper;
import com.forum.model.User;
import com.forum.util.MyConstant;
import com.forum.util.MyUtil;
...
public class LoginService {
    ...
    public String register(User user,String repassword){
        ...
        user.setActicateCode(activateCode);
        user.setJoinTime(MyUtil.formatDate(new Date()));
        user.setUsername("Cydar"+new Random().nextInt(1000)+"号");
    ...
    public Map<String,Object> login(User user){
        ...
        if(checkActived==0){
            map.put("status","no");
            map.put("error","您还没有激活账户，请前往邮箱激活");
            return map;
        }
